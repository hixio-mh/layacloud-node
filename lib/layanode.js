...
const lutil = require('./common/util');
 ...
 * @return {Promise}
 */
LayaNode.prototype.send = async function (node, msg) {
    let peer = null
    if(node && node.isPeer && node.isPeer() == true) {
        peer = node
    } else {
        peer = new Peer(node.node_hash_address, node.ip_address, node.http_port)
    }
    let url = format('{}/{}', peer.httpurl(), msg.type);
    return httpclient.post(url, JSON.stringify(msg))
};
 ...
 * @return {Promise}
 */
LayaNode.prototype.signAndSend = async function (node, msg) {
    let peer = null
    if(node && node.isPeer && node.isPeer() == true) {
        peer = node
    } else {
    ...
    let url = format('{}/{}', peer.httpurl(), msg.type);
    // FIXME: 修改toBuffer还是get_local_private， 不要这么机械的通过添加 0x 来实现 by litao
    let privateKey = lutil.toBuffer("0x" + nodeKey.get_local_private())
    return httpclient.post(url, packetManager.packet(privateKey, msg));
};
