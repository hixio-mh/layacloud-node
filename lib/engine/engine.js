...
Engine.prototype.onclose = function() {
  ...
  this._game.engineData.saveUserToStorage(this._userList)
...
Engine.prototype.enterRoom = async function(userId) {
  logger.debug("engine 玩家:%s 进入房间:%s ! userList:", userId, this._room.id, this._userList)
...
Engine.prototype.recvEventFromLogic = function(frameCount, type, data) {
  if(this._isSupervisor) {
    logger.debug("监督节点 房间:%s 添加待执行的逻辑事件, 类型:%s 值:", this._room.id, type, data)
