...
Engine.prototype.init = function() {
  ...
  try {
    ...
    _room.syncEventToSup("oncreated", [])
  } catch(e) {
  ...
  if(this._isSupervisor) {
    this._supervisor = new EngineSupervisor()
    loopCb = this._supervisor.onLoop()
    // 监督节点的loop要慢
    interval += 9000
  } else {
    loopCb = function() {
      if(this._runUpdateLoop) {
        _room.syncEventToSup("onupdate", [])
...
Engine.prototype.onClientMsg = function(userId, key, value) {
  ...
  if(key == "room.startgame") {
    ...
    _room.syncEventToSup("onuserevent", [userId, key, value])
  }
}
...
Engine.prototype.enterRoom = function(userId) {
  ...
  _room.syncEventToSup("onuserin", [userId, data])
  return true
}
...
Engine.prototype.leaveRoom = function(userId) {
  ...
  _room.syncEventToSup("onuserout", [userId])
  return true
}
...
Engine.prototype._startLoop = function(interval, cb) {
  function ontimeout() {
    logger.debug("engine loop on timeout!")
    this._frameCount++
    if(cb) {
      cb()
    }
