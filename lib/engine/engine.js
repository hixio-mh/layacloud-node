...
Engine.prototype.init = function() {
  ...
  logger.debug("room:%s engine进行初始化, game:%s 房间类型:%s 监督节点:", 
    this._room.id, this._game.id, this._roomName, this._isSupervisor)
  try {
...
Engine.prototype.broadcast = function(data) {
  ...
  this._room.send(this._userList, data)
}
...
Engine.prototype.onclose = function() {
  ...
  this._room.syncEventToSup("onclose", [])
  this._userList = []
...
Engine.prototype.onClientMsg = function(userId, key, value) {
  ...
  if(key == "room.startgame") {
    ...
    this._room.syncEventToSup("onClientMsg", [userId, key, value])
  }
}
...
Engine.prototype.enterRoom = async function(userId) {
  logger.debug("room:%s engine调用玩家:%s 进入房间! userList:", this._room.id, userId, this._userList)
  if(this._userList.indexOf(userId) != -1) {
    return false
  }
  // 加载数据
  let data = await this._loadUserData(userId)
  if(!data) {
    return false
  }
...
Engine.prototype.leaveRoom = function(userId) {
  ...
  this._room.syncEventToSup("leaveRoom", [userId])
  return true
}
 ...
 * @param {*} userId 
 */
Engine.prototype._loadUserData = async function(userId) {
  let data = this._game.engineData.getUserData(userId)
  if(!data) {
    try{
      ...
      return this._game.engineData.initUserData(userId, data)
    } catch(e) {
