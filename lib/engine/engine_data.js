...
EngineData.prototype.saveUserToStorage = async function(userList) {
  logger.debug("保存用户的数据到storage:", userList)
  let signersList = await this._doSaveToStorage(userList)
  if(signersList.length > 0) {
    CenterHelper.on_battle_over_savedone(this.id, signersList[0])
  } else {
 ...
 * 并返回roomid签名
 */
EngineData.prototype._doSaveToStorage = function(userList) {
  let cap = app.layaNode.getCapability('logic')
