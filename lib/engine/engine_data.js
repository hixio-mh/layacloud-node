 ...
 */
EngineData.prototype.saveUserToStorage = async function(gameId, roomId, userList) {
  logger.debug("保存游戏:%s 房间:%s 用户数据到storage, 用户列表:", gameId, roomId, userList)
  let signersList = await this._doSaveToStorage(gameId, roomId, userList)
  if(signersList.length > 0 && signersList[0].length == 2) {
    let list = (signersList[0][1]).map(e => e.sig)
    CenterHelper.on_battle_over_savedone(this.id, list)
  } else {
...
EngineData.prototype._trySync = function(data, id, force = false) {
  let meta = null
  // logger.debug("尝试持久化数据 id:", id, "| 全部data:", data, "force:", force)
 ...
 * 并返回roomid签名
 */
EngineData.prototype._doSaveToStorage = function(gameId, roomId, userList) {
  let cap = app.layaNode.getCapability('logic')
  let pAll = userList.map((userId) => {
    ...
    return new Promise((resolve, reject) => {
      cap.write(gameId, roomId, userId, data, (err, cbData) => {
        let matchResult = app.gameMgr.matchResult
        matchResult.deleteRoomSupervisor(gameId, roomId)
        if(err) {
