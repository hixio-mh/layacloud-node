 ...
 * @param message
 */
LogicCapability.prototype.onStorageNodeProvision = async function(message) {
    logger.debug('获取存储节点列表', JSON.stringify(message, null, 4));
...
LogicCapability.prototype.write = function (userId, gameId, data, cb) {
    ...
    let verifyReq = {
        ...
        peer: {ip: app.args.addr, port: app.args.pport}
    };
...
LogicCapability.prototype.onMessageTestReadData = async function(message) {
    ...
    try {
       let data = await this.read(userId, gameId);
        return { error: 0, userId: userId, gameId: gameId, data: data};
    }
    catch (e) {
...
LogicCapability.prototype.onMessageTestWriteData = async function(message) {
    ...
    const writeFn = BB.promisify(this.write);
    try {
        let res = await writeFn(userId, gameId, data);
