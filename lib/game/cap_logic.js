...
LogicCapability.prototype.onAppointed = function (input) {
    ...
    matchResult.setRoomMaster(data.room_hash, master)
...
LogicCapability.prototype.onStorageNodeProvision = function (message) {
    ...
    if (snl) {
        let peers = snl.map(node => new Peer(node.node_hash_address, node.ip_address, node.http_port));
        logger.debug('storage nodes for user :', userId, peers);
        this.storagePeersMap.set(userId, peers);
    }
    else {
        logger.warn('no storage peers for user:', userId);
    }
};
...
LogicCapability.prototype.onMessageDataVerified = function (message) {
    ...
    const key = userId + gameId + digest;
...
LogicCapability.prototype.write = function (gameId, roomId, userId, data, cb) {
    ...
    if(!game) {
        setImmediate(function () {
            cb(new Error('gameId not exists ' + gameId), null);
        });
    ...
    const hash = util.bufferToHexWith0x(util.keccak(util.toBuffer(JSON.stringify(data))));
    const key = userId + gameId + hash;
    ...
    if (nodeIp === '0.0.0.0') { // try to get ip from local interface， TEST PURPOSE
...
LogicCapability.prototype.on_matched = function (input) {
    let data = input.data
    logger.info('on_matched 匹配完成，通知玩家:%s', data.notify_target_player);
    let gameId = data.game_id
    ...
    if(!user) {
        logger.warn("匹配成功时，玩家:%s 不再本节点中，忽略", userId)
        logger.warn(app.gameMgr.inspect2(gameId))
        return
    }
    // 玩家离开原有房间
    ...
    // FIXME: low b方案
    setTimeout(() => {
        let packet = PUtil.userToGame(gameId, userId, data)
        logger.debug("向玩家:%s 发送user.togame消息:", userId, packet)
        user.send(packet)
    }, 2000)
