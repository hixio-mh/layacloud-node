...
LogicCapability.prototype.onAppointed = function (input) {
    ...
    let ret = game.matchResult.storeResult(data.room_hash, data.player_pubkey_list, data.supervison_node_list)
    let role = app.consts.ROOM_ROLE.LOGIC
...
LogicCapability.prototype.onMessageDataVerified = async function (message) {
    ...
    if (this.pendingWriteMap.has(key)) {
        ...
        const roomId = gameMgr.matchResult.getRoomId(userId);
