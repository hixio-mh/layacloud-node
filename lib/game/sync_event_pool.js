...
SyncEventPool.prototype.stop = function() {
  ...
  logger.debug("房间:%s sync event pool停止", this._roomId)
...
SyncEventPool.prototype.addEvent = function(frameCount, type, data) {
  // logger.debug("房间:%s 添加等待同步事件, frame:%d 类型:%s 数据:\n", this._roomId, frameCount, type, data)
...
SyncEventPool.prototype._doSyncEvent = async function() {
  return await SupvervisorHelper.syncLogicEvent(this._gameId, this._roomId, this._list)
}
