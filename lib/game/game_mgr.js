...
const protocolOrder = require('./protocol_order.js')
...
function GameMgr() {
  ...
  this._protocolOrder = protocolOrder
...
GameMgr.prototype.onReqRoomJoin = async function(socket, packet) {
  ...
  return await this._doEnter(socket, packet, async (err, room) => {
    this._notifyPlayerOnline(gameId, userId)
    if(err) {
...
GameMgr.prototype._doEnter = async function (socket, packet, cb) {
  ...
  let userId = packet.params.userid
  // 加载代码
  let gameInfo = await this._gameCode.getGame(gameId)
  if(!gameInfo) {
...
GameMgr.prototype._trySendRoomJoined = function(room) {
  ...
  if(eqSet(set, setExpect)) {
    ...
    room.setState(app.consts.ROOM_STATE.START)
  }
}

GameMgr.prototype._notifyPlayerOnline = function(gameId, userId) {
  CenterHelper.player_online(gameId, userId, function(err, resp) {
    if(err) {
      logger.warn("提交用户online报错!error:", err)
      return
    }
  })
}
