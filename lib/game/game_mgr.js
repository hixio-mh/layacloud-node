...
GameMgr.prototype.enter = async function(socket, request) {
  let [valid, gameId, userId, args] = this.parseEnterUrl(request.url)
  if(!valid) {
  ...
  game.enter(userId, socket, request, args)
}
...
GameMgr.prototype.parseEnterUrl = function(url) {
  ...
  let [, gameId, userId, ...tail] = parts
  // TODO 检测game Id是否合法
  // TODO 判断user签名
  return [true, gameId, userId, tail]
}
...
GameMgr.prototype.loadGame = async function(gameId) {
  // TODO 
  try {
    let baseUrl = "http://localhost/" + gameId + "/"
    let configUrl = baseUrl + "config.json"
    let config = await this.requestFile(configUrl)
    if(!config) {
      logger.error("can't find game %s config file", gameId)
      return false
    }
    let jsUrl = baseUrl + "/common.js"
    let js = await this.requestFile(jsUrl)
    if(!js) {
      logger.error("can't find game %s code file", gameId)
      return false
    }
    let [valid, version, jsCode] = this.parseGameJs(js)
    if(!valid) {
      logger.error("check game %s source error!")
      return false
    }
    let game = new Game(gameId, version, JSON.parse(config), jsCode)
    if(!game.init()){
      return false;
    }
