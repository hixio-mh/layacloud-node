...
User.prototype._setupListener = function() {
  ...
  if(app.gameMgr.socketReuse.isExists(this.socket)) {
    return
  }
...
User.prototype._onclose = function(code) {
  ...
  app.gameMgr.socketReuse.delete(this.socket)
...
User.prototype.doLogout = function(reason = 0) {
  ...
  if(this.socket) {
    if(reason != app.consts.LEAVEROOM_REASON.ROUND_FINISH) {
      ...
      app.gameMgr.socketReuse.add(this.socket)
    }
  }
  if(this.roomId) {
...
User.prototype._notifyPlayerOffline = function() {
  ...
  CenterHelper.player_offline(this._game.id, this.id, this.roomId, function(err, resp) {
    if(err) {
