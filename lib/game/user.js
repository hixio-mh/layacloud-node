...
User.prototype.setRoomId = function(roomId = "") {
  logger.debug("设置用户:%s 的room id为:", this.id, roomId == ""?"空":roomId)
  this.roomId = roomId
}
...
User.prototype.updateSocket = function(socket) {
  if(!socket) {
    logger.info("用户:%s updateSocket时socket为空", this.id)
  }
  this.socket = socket
...
User.prototype.isDataLoaded = function() {
  return this.state == app.consts.USER_STATE.ENTERED 
...
User.prototype.send = function(data) {
  logger.debug("向用户:%s 发送数据:", this.id, data, "socket:", this.socket?"可用":null)
...
User.prototype._onReqRoomLeave = async function(packet) {
  ...
  if(reason == app.consts.LEAVEROOM_REASON.ROUND_FINISH) {
    logger.info("对局结束，玩家:%s 请求离开房间", this.id)
