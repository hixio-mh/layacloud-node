...
kconst UserMgr = require('./user_mgr.js')
const EngineData = require('../engine/engine_data.js')

function Game(gameId, version, config, JsCode) {
  this.id = gameId
...
Game.prototype.enter = function(userId, socket, request, args) {
  ...
  if(!user) {
    user = new User(this, userId, socket, request)
    this.userMgr.addUser(userId, user)
  }

  if(!(args && args.length > 0)) {
    logger.warn("user login ws url don't has room info")
    return false
  }
  let [roomId] = args
