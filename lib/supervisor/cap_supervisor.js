...
Supervisor.prototype.onAppointed = function (input) {
    ...
    let master = data.player_pubkey_list[0]
...
Supervisor.prototype.approveUserData = function (reqNode, userId, gameId, userDataDigest, ts) {
    ...
    let ret = digestCalc == userDataDigest?1:0
    logger.debug("监督节点证明数据:%s \n收到:%s \n计算:%s", ret==1?"一致":"不同", userDataDigest, digestCalc, "\n本地数据:", data)
    return ret
};
