...
Supervisor.prototype.approveUserData = function (sendnode, userId, userDataDigest, ts) {
...
Supervisor.prototype.startDataVerificationJob = function () {
    ...
    let run = function () {
        try {
            ...
            if (item) {
                // {sender: '0x...', nonce: n, data: {type: 'message_data_verify', userId: '', digest: '0x...', peer: {ip: '', port: n}}}
                let userId = item.data.userId;
                let userDataDigest = item.data.digest;
                let ts = item.data.ts;
                let sender = item.sender;

                if (that.approveUserData(sender, userId, userDataDigest, ts)) {
