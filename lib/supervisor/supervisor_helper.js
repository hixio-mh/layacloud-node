 ...
 * @param {*} list 
 */
SupervisorHelper.syncEvent = async function (supNodeList, gameId, roomId, list) {
  return this._doSyncEvent(supNodeList, "sync_logic_event", gameId, roomId, list)
}
...
SupervisorHelper.syncSysEvent = async function (gameId, userId, type, data) {
  ...
  if(!roomId) {
    logger.warn("同步事件,无法找到game:%s 玩家:%s 的房间信息", gameId, userId)
    return false
  }
 ...
 * @param {*} list
 */
SupervisorHelper._doSyncEvent = async function(supNodeList, type, gameId, roomId, list) {
  logger.debug("同步房间:%s 事件到监督节点:", roomId, supNodeList)
  let node = app.layaNode
  // FIXME: 这里传递game_id和room_id不安全
  let msg = {type: type, game_id:gameId, room_id:roomId, list: list}
