 ...
 */
Worker.player_offline = function(gameId, userId,room_id, cb) {
    var packet = {}
    packet.player_pubkey = userId;
    packet.game_id = gameId;
    packet.room_hash = room_id;
    logger.debug("提交玩家下线:", app.inspect(packet))
 ...
 * 玩家在线，通知
 */
Worker.player_online = function(gameId, userId, cb) {
    var packet = {}
    packet.player_pubkey = userId;
    packet.game_id = gameId;
    logger.debug("提交玩家上线:", app.inspect(packet))
    Worker.post_to_center('player_online', packet, function (err, res) {
        if (cb) {
            cb(err, res);
        }
    })
