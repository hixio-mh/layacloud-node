...
Storage.prototype.onAppointed = async function (data) {
    ...
    logger.debug('任命为存储节点 userid = %s', userId);
};
...
Storage.prototype.readFromETH = async function (userId, gameId) {
    try {
        ...
        logger.debug('read ETH for user %s, got data: %o', userId, d);
...
Storage.prototype.onCreatePermission = function (message) {
    ...
    logger.debug('创建权限 %o', this.permissionMap);
};
...
Storage.prototype.onAddPermission = function (message) {
    ...
    this.addPermission(userId, l, message.data.ttl);

    logger.debug('收到存储权限%o，用户%s，更新权限 %o', message, userId, this.permissionMap);
};
                    ...
                    co(function* () {
                        ...
                        if (!request_done.online) {
                            ...
                            if (data.d) {
                                ...
                                console.log("[上链流程]:startWrite2ChainJob NodeKey.get_local_private",nodeKey.get_local_private());
                                let sig = eth.sign(util.sha3(data), nodeKey.get_local_private());

                                console.log("[上链流程]:startWrite2ChainJob sig",sig);
